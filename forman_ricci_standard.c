...
// =========================================================================
// Standard Forman-Ricci curvature (1-dimensional, no triangle contributions)
// Based on Equation 4.9 from JMLR 2025 paper
// Simpler formula: Ric_F(e) = 4 - deg(v1) - deg(v2) for unweighted case
// Weighted: Ric_F(e) = w(v1) + w(v2) - w(e) * sum_parallel_edges(sqrt(w_v/(w_e*w_e')))
// =========================================================================
void calc_standard_frc(Graph *g) {
    for (int e = 0; e < g->n_edges; e++) {
        int v1 = g->edge_src[e];
        int v2 = g->edge_dst[e];
        ...
        // Sum over ALL parallel edges from v1 (excluding edge to v2)
        double sum1 = 0.0;
        for (int j = 0; j < g->v_adj_length[v1]; j++) {
            int n = g->v_adj_list[g->v_adj_begin[v1] + j];
            if (n == v2) continue;
            double w_e_prime = g->edge_weight[g->v_adj_begin[v1] + j];
            sum1 += sqrt(w_v1 / (w_e * w_e_prime));
        }
        
        // Sum over ALL parallel edges from v2 (excluding edge to v1)
        double sum2 = 0.0;
        for (int j = 0; j < g->v_adj_length[v2]; j++) {
            int n = g->v_adj_list[g->v_adj_begin[v2] + j];
            if (n == v1) continue;
            double w_e_prime = g->edge_weight[g->v_adj_begin[v2] + j];
            sum2 += sqrt(w_v2 / (w_e * w_e_prime));
        }
        
        // Standard Forman-Ricci (no triangles)
        double curv = w_v1 + w_v2 - w_e * (sum1 + sum2);
...
void update_weights(Graph *g, double step_size) {
    for (int e = 0; e < g->n_edges; e++) {
        ...
        if (w_new < 1e-6) w_new = 1e-6;
        if (w_new > 10.0) w_new = 10.0;
...
// =========================================================================
// Connected components - for Standard FRC (bridges have LOW weight)
// Keep edges ABOVE threshold to cut low-weight inter-cluster bridges
// =========================================================================
    ...
    for (int start = 0; start < g->n_nodes; start++) {
        ...
        while (q_start < q_end) {
            ...
            for (int j = 0; j < g->v_adj_length[node]; j++) {
                ...
                // For Standard FRC: Keep edges ABOVE threshold
                // (bridges have LOW weight after Ricci flow)
...
int main() {
    ...
    printf("=== Forman-Ricci Clustering (Standard FRC - No Triangles) ===\n");
    ...
    // Ricci flow
    printf("\n=== Ricci Flow (Standard Forman-Ricci - No Triangles) ===\n");
