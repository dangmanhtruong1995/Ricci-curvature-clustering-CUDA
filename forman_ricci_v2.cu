...
// =========================================================================
// IMPROVED Forman-Ricci Curvature
// Key insight: Use the DIFFERENCE in clustering coefficient, not absolute triangles
// =========================================================================
__global__ void calc_forman_curvature_improved(
        int *d_v_adj_list, int *d_v_adj_begin, int *d_v_adj_length,
        int *d_edge_src, int *d_edge_dst,
        char *d_faces, int *d_n_triangles,
    ...
    while (idx < n_undirected_edges) {
        ...
        // Find edge indices in adjacency list
        ...
        // Count triangles and non-shared neighbors
        int n_triangles = 0;
        int n_v1_only = 0;
        int n_v2_only = 0;
        ...
        // =====================================================================
        // IMPROVED CURVATURE FORMULA
        // =====================================================================
        // 
        // The standard Forman formula: 4 - deg(v1) - deg(v2) + 3*#triangles
        // Problems: Depends heavily on absolute degree, doesn't normalize well
        //
        // Our improved formula focuses on LOCAL CLUSTERING around the edge:
        //
        // For an edge (v1, v2):
        //   - max_triangles = min(deg(v1)-1, deg(v2)-1)  
        //   - triangle_density = #triangles / max_triangles
        //
        // Intra-cluster edges: High triangle density (neighbors are connected)
        // Inter-cluster edges: Low triangle density (neighbors in different clusters)
        //
        // Curvature = 2 * triangle_density - 1
        // Range: [-1, +1]
        //   - Positive for dense (intra-cluster) edges
        //   - Negative for sparse (inter-cluster) edges
        
        int max_possible = min(deg_v1 - 1, deg_v2 - 1);
        
        float curvature;
        if (max_possible <= 0) {
            // Edge endpoint has degree 1, can't form triangles
            curvature = -1.0f;  // Treat as bridge
        } else {
...
// =========================================================================
// Weight update with Ricci flow
// =========================================================================
__global__ void update_weight_ricci_flow(
        int *d_v_adj_list, int *d_v_adj_begin, int *d_v_adj_length,
        int *d_edge_src, int *d_edge_dst,
        float *d_edge_weight, float *d_edge_curvature,
    ...
    while (idx < n_undirected_edges) {
        ...
        // Ricci flow: dw/dt = -curvature * w
        // Discretized: w_new = w * (1 - step * curvature)
        //
        // Positive curvature (intra-cluster) -> weight DECREASES
        // Negative curvature (inter-cluster) -> weight INCREASES
        //
        // After flow: intra edges have LOW weight, inter edges have HIGH weight
        
        float w_old = d_edge_weight[idx_v1_v2];
        float curv = d_edge_curvature[idx_v1_v2];
        float w_new = w_old * (1.0f - step_size * curv);
        
        // Clamp to prevent numerical issues
        w_new = fmaxf(w_new, 1e-8f);
        w_new = fminf(w_new, 1e8f);
...
// =========================================================================
// IMPROVED: Edge-based clustering using sorted weights
// Instead of BFS with threshold, we:
// 1. Sort edges by weight
// 2. Remove top X% of edges (highest weight = bridges)
// 3. Find connected components in remaining graph
// =========================================================================

// CPU-based connected components on sparse graph (after edge removal)
int find_components_with_removed_edges(
    ...
    for (int start = 0; start < n_nodes; start++) {
        ...
        while (q_start < q_end) {
            ...
            for (int j = 0; j < v_adj_length[node]; j++) {
                ...
                // Only traverse edges with weight BELOW threshold
                // (Low weight = intra-cluster, high weight = inter-cluster bridge)
...
int main() {
    ...
    printf("=== Forman-Ricci Clustering v2 ===\n");
    printf("Nodes: %d, Clusters: %d\n", N_NODE, N_CLUSTERS);
    printf("P_IN: %.4f, P_OUT: %.6f\n\n", P_IN, P_OUT);
    ...
    float intra_tri = 0, inter_tri = 0;
    for (int i = 0; i < n_undirected_edges; i++) {
        if (get_cluster(edge_src[i]) == get_cluster(edge_dst[i])) {
            intra_tri += h_n_triangles[i];
        } else {
            inter_tri += h_n_triangles[i];
        }
    }
    printf("Avg triangles: intra=%.2f, inter=%.4f\n\n", 
           intra_tri/intra_count, inter_tri/inter_count);

    // =========================================================================
    // Ricci Flow
    // =========================================================================
    printf("=== Ricci Flow (%d iterations) ===\n", N_ITERATION);
    
    for (int iter = 0; iter < N_ITERATION; iter++) {
        n_block = (n_undirected_edges + BLOCK_SIZE - 1) / BLOCK_SIZE;
        
        calc_forman_curvature_improved<<<n_block, BLOCK_SIZE>>>(
            d_v_adj_list, d_v_adj_begin, d_v_adj_length,
        ...
        for (int i = 0; i < n_total_edge; i++) {
            if (fabsf(h_edge_curvature[i]) > max_curv) 
                max_curv = fabsf(h_edge_curvature[i]);
        }
        
        // More aggressive step size for faster convergence
        float step = 0.8f / (max_curv + 0.01f);
        step = fminf(step, 0.8f);

        update_weight_ricci_flow<<<n_block, BLOCK_SIZE>>>(
            d_v_adj_list, d_v_adj_begin, d_v_adj_length,
            d_edge_src, d_edge_dst,
            d_edge_weight, d_edge_curvature,
        ...
        if (iter % 20 == 0 || iter == N_ITERATION - 1) {
            printf("Iter %3d: max_curv=%.3f, step=%.3f\n", iter, max_curv, step);
        }
    }
    ...
    // Collect weights for each edge (undirected)
    ...
    for (int i = 0; i < n_undirected_edges; i++) {
        ...
        if (get_cluster(v1) == get_cluster(v2)) {
            intra_sum += w;
        } else {
            inter_sum += w;
        }
    ...
    printf("\n=== Weight Analysis ===\n");
    printf("Avg weight: intra=%.6f, inter=%.6f\n", intra_sum/intra_count, inter_sum/inter_count);
    printf("Separation ratio: %.2fx\n", (inter_sum/inter_count)/(intra_sum/intra_count));

    // Sort weights to find percentiles
    float *sorted_weights = (float*)malloc(n_undirected_edges * sizeof(float));
    memcpy(sorted_weights, edge_weights, n_undirected_edges * sizeof(float));
    qsort(sorted_weights, n_undirected_edges, sizeof(float), compare_float);

    printf("\nWeight percentiles:\n");
    int p10 = (int)(0.10 * n_undirected_edges);
    int p50 = (int)(0.50 * n_undirected_edges);
    int p90 = (int)(0.90 * n_undirected_edges);
    int p95 = (int)(0.95 * n_undirected_edges);
    int p99 = (int)(0.99 * n_undirected_edges);
    printf("  10%%: %.6f\n", sorted_weights[p10]);
    printf("  50%%: %.6f\n", sorted_weights[p50]);
    printf("  90%%: %.6f\n", sorted_weights[p90]);
    printf("  95%%: %.6f\n", sorted_weights[p95]);
    ...
    // =========================================================================
    // IMPROVED: Use percentile-based threshold
    // Since we know inter_count, use that as guide
    // =========================================================================
    printf("\n=== Clustering with Percentile Thresholds ===\n");
    ...
    int best_n_clusters = 0;
    
    // Try percentile thresholds from 80% to 99.9%
    // We want to cut the top X% of edges (highest weight = bridges)
    float percentiles[] = {0.80, 0.85, 0.90, 0.92, 0.94, 0.95, 0.96, 0.97, 0.98, 0.99, 0.995, 0.999};
    int n_percentiles = sizeof(percentiles) / sizeof(percentiles[0]);
    
    printf("%-10s %-10s %-10s %-10s %-10s\n", "Percentile", "Threshold", "Clusters", "Modularity", "NMI");
    printf("--------------------------------------------------------\n");
    
    for (int p = 0; p < n_percentiles; p++) {
        int idx = (int)(percentiles[p] * n_undirected_edges);
        if (idx >= n_undirected_edges) idx = n_undirected_edges - 1;
        float threshold = sorted_weights[idx];
        
        int n_clusters = find_components_with_removed_edges(
            v_adj_list, v_adj_begin, v_adj_length,
            h_edge_weight, threshold, component_id, N_NODE);
        
        if (n_clusters < 2 || n_clusters > N_NODE / 10) continue;
        
        float modularity = calculate_modularity(
            v_adj_list, v_adj_begin, v_adj_length,
            component_id, N_NODE, n_undirected_edges);
        
        float nmi = calculate_nmi(component_id, node_cluster, N_NODE, n_clusters, N_CLUSTERS);
        
        printf("%-10.1f%% %-10.6f %-10d %-10.4f %-10.4f", 
               percentiles[p] * 100, threshold, n_clusters, modularity, nmi);
    ...
    // =========================================================================
    // Also try absolute threshold search
    // =========================================================================
    printf("\n=== Fine-grained Threshold Search ===\n");
    
    float min_w = sorted_weights[0];
    float max_w = sorted_weights[n_undirected_edges - 1];
    
    for (int i = 0; i <= 50; i++) {
        float threshold = min_w + (max_w - min_w) * i / 50.0f;
        
        int n_clusters = find_components_with_removed_edges(
            v_adj_list, v_adj_begin, v_adj_length,
            h_edge_weight, threshold, component_id, N_NODE);
        
        if (n_clusters < 2 || n_clusters > 50) continue;
        ...
        if (n_clusters >= 5 && n_clusters <= 20) {
            printf("threshold=%.6f: %d clusters, Q=%.4f, NMI=%.4f", 
                   threshold, n_clusters, modularity, nmi);
    ...
    printf("========================================\n");
    printf("Best threshold: %.6f\n", best_threshold);
    printf("Clusters found: %d (ground truth: %d)\n", best_n_clusters, N_CLUSTERS);
    printf("Modularity: %.4f\n", best_modularity);
    printf("NMI: %.4f\n", best_nmi);

    // Get final clustering with best threshold
    find_components_with_removed_edges(
        v_adj_list, v_adj_begin, v_adj_length,
        h_edge_weight, best_threshold, component_id, N_NODE);

    // Print cluster sizes
    int *sizes = (int*)calloc(best_n_clusters, sizeof(int));
    for (int i = 0; i < N_NODE; i++) {
        if (component_id[i] >= 0 && component_id[i] < best_n_clusters) {
            sizes[component_id[i]]++;
        }
    }
    
    printf("\nCluster sizes:\n");
    for (int c = 0; c < best_n_clusters && c < 15; c++) {
        printf("  Cluster %d: %d nodes\n", c, sizes[c]);
    }
    if (best_n_clusters > 15) printf("  ... (%d more)\n", best_n_clusters - 15);
